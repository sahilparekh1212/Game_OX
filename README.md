...
A clean, responsive Tic Tac Toe built with **TypeScript** + **Vite** — play a friend or the CPU.
...
- **Mode** — **vs CPU** or **2 Players** on the same device. In 2‑player, enter **player names** and the scoreboard/status use them.
- **Play as** (vs CPU) — choose **X** or **O**.
- **First move** (vs CPU) — choose who opens: **You**, **CPU**, or **🪙 Flip** for a coin toss decided fresh each game (the status line announces who won the flip).
- **Difficulty** (vs CPU):
  - **Easy** — plays random moves.
  - **Medium** — takes a winning move, blocks yours, otherwise random.
  - **Hard** — **minimax** play with an occasional random slip: it always takes an open win and rarely errs, but a sharp opponent can catch it out — strong, not unbeatable.
